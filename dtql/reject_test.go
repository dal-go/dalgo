...
func TestSerialize_rejectsOutOfScopeConstructs(t *testing.T) {
	...
	tests := []struct {
		...
		{"having", fakeQuery{from: rootFrom(), having: dal.WhereField("count", dal.GreaterThen, 1)}, "Having is not supported"},
		{"cursor", fakeQuery{from: rootFrom(), startFrom: dal.Cursor("c1")}, "cursor (StartFrom) is not supported"},
		{"start after", fakeQuery{from: rootFrom(), startAfter: dal.Cursor("c2")}, "cursor (StartAfter) is not supported"},
		{"negative limit", fakeQuery{from: rootFrom(), limit: -1}, "non-negative"},
		{"negative offset", fakeQuery{from: rootFrom(), offset: -1}, "non-negative"},
		{"qualified field", fakeQuery{from: rootFrom(), columns: []dal.Column{{Expression: dal.NewFieldRef("u", "name")}}}, "qualified field"},
		{"wildcard mixed with expression", fakeQuery{from: rootFrom(), columns: []dal.Column{{Expression: dal.Field("id"), Wildcard: &dal.WildcardProjection{Exclude: []string{"email"}}}}}, "mixes wildcard and expression"},
		{"wildcard with alias", fakeQuery{from: rootFrom(), columns: []dal.Column{{Alias: "rest", Wildcard: &dal.WildcardProjection{Exclude: []string{"email"}}}}}, "cannot have an alias"},
		{"wildcard without exclusions", fakeQuery{from: rootFrom(), columns: []dal.Column{dal.AllColumnsExcept()}}, "must contain at least one"},
		{"wildcard unknown source", fakeQuery{from: rootFrom(), columns: []dal.Column{dal.AllColumnsExceptFrom("x", "email")}}, "does not match from name or alias"},
		{"map constant", fakeQuery{from: rootFrom(), columns: []dal.Column{{Expression: dal.Constant{Value: map[string]any{"x": 1}}}}}, "unsupported constant"},
		{"nested array", fakeQuery{from: rootFrom(), columns: []dal.Column{{Expression: dal.Array{Value: []any{[]any{1}}}}}}, "unsupported array"},
