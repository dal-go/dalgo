...
Implements the `typed-collection-extras` Feature: the opt-in `dal.ManyInserter[T]` batch-insert interface (with dal-native `Item[T]`) plus the `Count`/`Exists`/`First` read conveniences on `dal.Collection[T]`, all additive over existing `dal` interfaces and cycle-free. Depends on the `typed-collection` Feature.

## Approach

Each terminal is independent (it adds one method/interface on the existing `Collection[T]` and is tested against `dalgo2memory`), so the tasks are ordered for review convenience rather than hard dependency: batch first, then the three read conveniences. `Count`/`First` reuse the same `StructuredQuery` plumbing as `All` (from `typed-collection`); `Exists` is a key-only probe; `InsertMany` builds complete keys from `Item.ID`. Linear, no parallel branches.

## Tasks

### Task 1: Item[T] type and ManyInserter[T] batch insert
