...
| [Schema API: subcollection declaration and driver options for CreateCollection](schema-subcollections-extensions/README.md) | Draft | Nested collections as `SchemaPath` (structured `Parent` steps or odd-segment strings like `ext/datatug/projects/{projectID}/queries`) in DALgo's key-path grammar; minimal three-method `SchemaModifier`; driver-specific settings as functional options (required refused / hint ignored); drops refuse data loss unless `DeleteRecords`/`DeleteNested` are given. |
