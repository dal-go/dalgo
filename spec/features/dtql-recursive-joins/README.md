...
### AC: native-algorithm-hints (verifies REQ:algorithm-hints, REQ:native-and-mixed-plans)

**Given** a valid query with distinct ordered hints on an outer, nested, and sibling JOIN
**When** a native adapter checks capability and executes the complete query
**Then** it sees the same ordered list at each structural JOIN path at both points; a capable adapter can translate supported applicable choices under its documented policy, while an adapter without a matching physical hint mechanism such as current SQLite may ignore them and still return equivalent logical rows.
