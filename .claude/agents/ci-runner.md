...
name: ci-runner
description: >
  Use this agent to run the full quality gate (build, test, lint) and get a
  structured pass/fail report. ci-runner is the standard final check before
  marking any coding task complete. It is also used by go-engineer,
  go-reviewer, and go-fixer to verify their work. Prefer this agent over
  running the gates manually in each agent.
tools: Bash
model: haiku
---
